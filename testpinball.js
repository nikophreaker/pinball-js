...
class PlayGame extends Phaser.Scene {
    ...
    createStaticBodies() {
        ...
        this.matter.world.add([
            ...
            this.boundary(this.gameWidth / 2, this.gameHeight, this.gameWidth, 20 * dpr, "bottom"),
            this.boundary(0, this.gameHeight / 2, 20 * dpr, this.gameHeight, "left"),
            this.boundary(this.gameWidth, this.gameHeight / 2, 10 * dpr, this.gameHeight, "right"),
    ...
    createPinball() {
        ...
        this.matter.world.on("collisionstart", function (event) {
            ...
            event.pairs.forEach(function (item, index) {
                if (item.bodyB.label == 'pinball') {
                    switch (item.bodyA.label) {
                        case 'bottom':
                            isFalling = true;
                            break;
                        case 'bumper':
    ...
    createEvents() {
        ...
        this.matter.world.on('beforeupdate', function (event) {
            ...
            //     });
            // }
            // let x = Math.max(Math.min(pinball.body.velocity.x, maxSpeed), -maxSpeed);
            // let y = Math.max(Math.min(pinball.body.velocity.y, maxSpeed), -maxSpeed);
            // scene.matter.body.setVelocity(pinball.body, {
            //     x: x,
            //     y: y,
            // });
    ...
    launchPinball() {
        // updateScore(0);
        this.matter.body.setPosition(pinball, {
            x: this.gameWidth / 1.2,
            y: this.gameHeight / 1.2
        });
        this.matter.body.setVelocity(pinball, {
            x: 0,
            y: -25 + this.rand(-2, 2)
        });
